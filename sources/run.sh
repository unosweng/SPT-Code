...
nohup python main.py --do-pre-train --pre-train-tasks cap,mass,mng --batch-size 16 --eval-batch-size 32 --cuda-visible-devices 0 --fp16 --model-name pre_train --n-epoch 1 --n-epoch-pre-train 1 --remove-existing-saved-file pre_train:fine_tune --pre-train-subset-ratio 0.1 --parse-subset-ratio 0.1 --task completion > output-spt-code-2024-$DATE.log 2>&1 &
