...
nohup python main.py --do-pre-train --pre-train-tasks mass --batch-size 16 --eval-batch-size 32 --cuda-visible-devices 0 --fp16 --model-name pre_train --pre-train-parse-subset-ratio 0.01 --train-subset-ratio 0.001 --n-epoch 1 --n-epoch-pre-train 1 --remove-existing-saved-file > output-spt-code-2024-$DATE.log 2>&1 &
